...
const AddVentaModal: React.FC<AddVentaModalProps> = ({
  visible,
  onCancel,
  onSave,
}) => {
  ...
  const handleAddProduct = () => {
    if (selectedProductId && selectedQuantity > 0) {
      ...
      if (productoSeleccionado) {
        ...
        const newMontoTotal =
          montoTotal + productoSeleccionado.precio_prod * selectedQuantity;
  ...
  const handleRemoveProduct = (productId: number) => {
    ...
    if (productoParaRemover) {
      ...
      const newMontoTotal =
        montoTotal - productoParaRemover.precio * productoParaRemover.cantidad;
  ...
  return (
    ...
    >
      <div>
        <label>Fecha*</label>
        <DatePicker style={{ width: "100%" }} value={fecha} disabled />
      </div>

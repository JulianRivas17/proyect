...
const ViewProduct: React.FC = () => {
    ...
    const { addToCart } = useCart(); // Mover al inicio del componente
    ...
    const handleIncrement = () => {
      ...
      setCount(newCount);
      setTotalPrice(newCount * (producto?.precio_prod || 0)); // Actualizar precio total
    };
    ...
    const handleAddToCart = () => {
      if (producto) {
        const item = {
          ...
          imagen: obtenerURLImagen(producto.image_url)?.toString() || "",
          descripcion: producto.description,
        };
        addToCart(item); // Añadir al carrito
        navigate("/landing"); // Navega al carrito después de agregar
      }
    };
  
    const urlImagen = producto ? obtenerURLImagen(producto.image_url)?.toString() : "";
    ...
    return (
      <Layout className="container-principal">
        ...
        {/* Carrito dinámico */}
        <div className="shopping-card-modal view">
          ...
          <div className="add-order">
            ...
            <Button className="add-Product-Order" onClick={handleAddToCart}>
              Agregar
            </Button>
          </div>
        </div>
